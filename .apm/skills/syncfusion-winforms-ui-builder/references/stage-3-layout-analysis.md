...
**Purpose:** Analyze user requirements, create optimal control-mapping.json, execute mapping script, and produce Syncfusion WinForms control assignments. **FULLY AUTOMATED WITH MANDATORY SCRIPT EXECUTION.**

---

## ⚠️ Critical: Skill Labels vs. NuGet Package Names

Reference labels in `controls.csv` (e.g., `syncfusion-winforms-datagrid`) are for BM25 search **only** — not official NuGet names.

| Control Class | Skill Label | Official NuGet |
|---|---|---|
| `SfDataGrid` | `syncfusion-winforms-datagrid` | `Syncfusion.SfDataGrid.WinForms` |
| `SfTextBoxExt` | `syncfusion-winforms-textboxext` | `Syncfusion.Tools.Windows` |
| `SfChart` | `syncfusion-winforms-chart` | `Syncfusion.SfChart.WinForms` |
| `ButtonAdv` | `syncfusion-winforms-button` | `Syncfusion.Tools.Windows` |

Stage 3 generates skill labels. Stage 6 resolves them to official NuGet packages via control skill files (`.apm/skills/syncfusion-winforms-<name>/SKILL.md`).
...
- Identify & Validate All Forms/UserControls from Intent (MANDATORY)
- Read control type, forms, and interactions from Stage 1 intent output
- List all distinct forms/usercontrols/panels (e.g., `LoginForm`, `DashboardUserControl`)
- Document form hierarchy and navigation flow (e.g., MDI parent/child, panel switching, dialog ownership)
- Identify all UI elements per form/usercontrol with their purpose
...
| Check | Rule |
|---|---|
| All forms/usercontrols listed | Every form/usercontrol from Stage 1 intent must be named |
| No missing screens | Zero gaps between intent and mapping |
| Unique element IDs | No duplicate `id` values across all forms/usercontrols |
| Type hints present | Every element has a descriptive `type_hint` |
| Naming consistent | Same suffix used throughout: `Form`, `UserControl`, or `Control` |
| View definitions complete | Every form/usercontrol has `page_id`, `component_type`, `title`, and `elements`/`sections` |
...
```json
{
  ...
  "layout_variant": "Multi-Form Application",
  "pages": [
    {
      "page_id": "login_form",
      "component_type": "generate_form",
      "title": "Login Form",
      "elements": [
...
| **< 10** | Use `NATIVE_WINFORMS` fallback — do NOT accept a weak Syncfusion substitution |
| **0** | Use `NATIVE_WINFORMS` with documented reason |

**Rules:**
- ✅ Map to the exact Syncfusion control class name
- ✅ Use `NATIVE_WINFORMS` when no valid Syncfusion control exists (with `fallback_reason` + `equivalent_native`)
- ❌ Never substitute with a different Syncfusion control (e.g., do not use `SfButton` in place of a missing checkbox)
- ❌ Never use native WinForms controls when a valid Syncfusion control exists

**NATIVE_WINFORMS fallback format:**
```json
{
  "element_id": "checkbox_example",
  "control": "NATIVE_WINFORMS",
  "skill": "native-winforms-checkbox",
  "fallback_reason": "No Syncfusion checkbox control available",
  "equivalent_native": "System.Windows.Forms.CheckBox"
}
```
...
- Script: `<project-root>/.apm/skills/syncfusion-winforms-ui-builder/scripts/controls_search.cjs`
...
5. Updates `control-mapping.json` with:
   ...
   - `skill` — Skill reference (e.g., `syncfusion-winforms-maskedtextbox`)
  ...
  "elements": [
    {
      ...
      "skill": "syncfusion-winforms-maskedtextbox",
      "score": 18.5,
      "validation": "✓ VERIFIED"
    }
  ],
...
|---|---|---|
| **Form Completeness** | All Stage 1 forms/usercontrols present in `control-mapping.json` | Re-run script or update JSON structure |
...
| **Score Validation** | Score > 10 for Syncfusion controls; Score 0 for NATIVE_WINFORMS fallbacks | Review low-score mappings |
...
| Rule | Why |
|---|---|
| **Skill labels ≠ NuGet packages** | Labels (e.g., `syncfusion-winforms-datagrid`) are search references only; NuGet resolution happens in Stage 6 |
| **All controls must map** | No "unmapped" elements allowed; use `NATIVE_WINFORMS` fallback with documented reason |
...
- Each element enriched with:
  ...
  - `skill` — Skill reference for NuGet resolution (e.g., `syncfusion-winforms-maskedtextbox`)
  - `score` — BM25 match confidence (0-100)
  - `validation` — "✓ VERIFIED" or "✗ FALLBACK"
