 ...
 *
 * Maps UI elements to Syncfusion WinForms controls + skills using BM25 semantic search.
...
/**
 * Map user queries to Syncfusion WinForms controls using BM25 search.
 */
class ControlMapper {
  /**
  ...
  /** Get all controls as [name, skill] pairs. */
  listAll() {
...
/**
 * Stage 3: Process control-mapping.json and enrich with Syncfusion WinForms control + skill mappings.
 *
 * Supports both:
...
/**
 * Map a single element to a Syncfusion WinForms control using BM25 search.
...
function mapElementToControl(element, mapper, onSuccess, onFallback) {
  ...
  if (results.length > 0) {
    ...
    if (verified && score > 10) {
      ...
      // ⛔ NEVER write an assumed control name — use NATIVE_WINFORMS only
      element.control = "NATIVE_WINFORMS";
      element.skill = null;
  ...
  if (args.length > 0) {
    ...
    try {
      ...
      if (enrichedJson.execution_metrics) {
        ...
        console.log(`   Fallback (NATIVE_WINFORMS): ${metrics.fallback_controls}`);
